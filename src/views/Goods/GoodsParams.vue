  ...
  export default {
    ...
    methods: {
      ...
      async getParamsData() {
        ...
        const res = await this.$API.goods.reqCateList(
          this.cateId,
          this.activeName
        )
        if (res.meta.status !== 200) {
      ...
      addParams() {
        ...
        this.$refs.addFormRef.validate(async (valid) => {
          if (!valid) {
            return this.$message.error('验证失败，请检查表单项！')
          }
          const res = await this.$API.goods.reqAddCateAttr(
            this.cateId,
      ...
      async showEditDialog(attr_id) {
        const res = await this.$API.goods.reqCateAttrById(
          this.cateId,
          attr_id,
          this.activeName
        )
      ...
      editParams() {
        this.$refs.editFormRef.validate(async (valid) => {
          if (!valid) {
            return this.$message.error('修改失败，请检查表单项！')
          }
          const res = await this.$API.goods.reqSubmitCateAttr(
            this.cateId,
      ...
      async removeParams(attr_id) {
        ...
        // 用户确认删除
        const res = await this.$API.goods.reqDeleteCateAttr(this.cateId, attr_id)
        if (res.meta.status !== 200) {
      ...
      async saveAttrVals(row) {
        // 需要发起请求，保存这次操作
        const res = await this.$API.goods.reqDeleteCateAttr(
          this.cateId,
          row.attr_id,
          row.attr_name,
          row.attr_sel,
